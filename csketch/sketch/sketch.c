...
shape_t *new_text(float x, float y, const char *text)
{
    struct text_data *data = (struct text_data *)malloc(sizeof(struct text_data));
    data->x = x;
    data->y = y;
    data->text = text;
    return new_shape(SHAPE_TEXT, data);
}
...
void write_shape(FILE *fp, shape_t *shape)
{
    ...
    case SHAPE_TEXT:
        x = shape->text_data->x;
        y = shape->text_data->y;
        fprintf(fp, "  <text x='%f' y='%f' transform='scale(1, -1)'>%s</text>\n", x, y, shape->text_data->text);
        break;
    }
}
